// Package huhstyle adapts charmbracelet/huh forms to the workspace-wide
// "Premium Dark-Tech" design tokens (Catppuccin Mocha).
//
// The 14 hex literals below are the only approved colors, and they are the
// ONLY hex literals allowed in this file: the same token set is used verbatim by
// the sibling Go front-ends in this workspace (ragcli, onnx2oracle, memoria,
// oragraphrag, emotion-engine), so a new shade here would fork the palette.
//
